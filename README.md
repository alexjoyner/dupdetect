...
<!-- [![Build Status](https://travis-ci.org/danielstjules/dupdetect.svg?branch=master)](https://travis-ci.org/danielstjules/dupdetect) -->
...
and repeated logic can be identified as well using dupdetect, since it
...
``` bash
npm install -g dupdetect
```

## Usage

```
Usage: dupdetect [options] <paths ...>


Detect copy-pasted and structurally similar JavaScript code
Example use: dupdetect -I -L -t 20 --ignore "test" ./path/to/src
...
Options:
  ...
  -c, --config [config]              path to config file (default: .dupdetectrc)
...
If a `.dupdetectrc` file is located in the project directory, its values will
...
```
dupdetect -t 50 --ignore "test" ./path/to/src
```
...
It's simple to run dupdetect on your library source as part of a build
...
To have dupdetect run with each job, but not block or fail the build, you can
use something like the following:

``` yaml
script:
  - "dupdetect ./path/to/src || true"
```
...
<file path="/dupdetect/spec/fixtures/intersection.js" line="1"/>
<file path="/dupdetect/spec/fixtures/intersection.js" line="7"/>
<codefragment>
